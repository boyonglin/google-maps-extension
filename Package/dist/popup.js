...
async function fetchData() {
    ...
    const { searchHistoryList = [], favoriteList = [], geminiApiKey = "", startAddr = "", authUser = 0, isIncognito = false, videoSummaryToggle = false, } = await getWarmState();
    if (searchHistoryList.length) {
        ...
        searchHistoryList.forEach((item) => frag.appendChild(history.createListItem(item, favoriteList)));
        ul.appendChild(frag);
        searchHistoryListContainer.appendChild(ul);
        const first = searchHistoryListContainer.querySelector(".list-group .list-group-item:first-child");
