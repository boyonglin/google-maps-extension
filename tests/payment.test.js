...
describe('Payment Component - Full Coverage', () => {
    ...
    beforeEach(() => {
        ...
        global.premiumNoteElement = premiumNoteElement = document.querySelector('p[data-locale="premiumNote"]');
    ...
    describe('updateNoteDisplay - isTrial', () => {
        test('should display trial note with end date and modal links', () => {
            ...
            expect(global.modal.text2Modal).toHaveBeenCalledWith('premiumNote', 'Alt+S / ⌥+S', 'tipsModal');
    ...
    describe('updateNoteDisplay - isPremium', () => {
        test('should display premium note with multilingual feedback links', () => {
            ...
            expect(premiumNoteElement.innerHTML).toBe('Thank you for being a premium user! 回饋 feedback フィードバック');
            ...
            expect(global.modal.text2Link).toHaveBeenCalledWith('premiumNote', 'フィードバック', feedbackUrl);
    ...
    describe('updateNoteDisplay - isFree', () => {
        test('should display free user note with ExtensionPay link', () => {
            ...
            expect(global.modal.text2Link).toHaveBeenCalledWith('premiumNote', 'ExtensionPay', 'https://extensionpay.com/');
    ...
    describe('updateNoteDisplay - Priority Testing', () => {
        test('should prioritize isFirst > isTrial > isPremium > isFree', () => {
            ...
            global.state.paymentStage = { isTrial: true, isPremium: true, isFree: true, trialEnd: Date.now() };
            paymentInstance.updateNoteDisplay();
        ...
        test('should handle all flags false gracefully', () => {
            global.state.paymentStage = { isFirst: false, isTrial: false, isPremium: false, isFree: false };
    ...
    describe('calcTrialEndDate', () => {
        test('should format date in US locale with 24-hour time', () => {
            expect(paymentInstance.calcTrialEndDate(new Date('2025-11-15T14:30:00').getTime())).toMatch(/Nov 15, 14:30/);
            expect(paymentInstance.calcTrialEndDate(new Date('2025-01-05T08:15:00').getTime())).toMatch(/Jan 5, 08:15/);
            expect(paymentInstance.calcTrialEndDate(new Date('2025-03-10T14:00:00').getTime())).toContain('14:00');
        });

        test('should handle edge case timestamps', () => {
            // Midnight (timezone dependent: 00:00 or 24:00)
            expect(paymentInstance.calcTrialEndDate(new Date('2025-12-25T00:00:00').getTime())).toMatch(/Dec 25, (00:00|24:00)/);
        ...
        test('should be deterministic for same timestamp', () => {
            ...
            expect(paymentInstance.calcTrialEndDate(timestamp)).toBe(paymentInstance.calcTrialEndDate(timestamp));
        });
    });
    ...
    describe('Integration - Full Payment Flow', () => {
        test('should handle trial user flow: unlock shortcuts and show trial note', () => {
            ...
            Array.from(shortcutTip).forEach(el => expect(el.classList.contains('premium-only')).toBe(false));
            expect(paymentSpan.innerHTML).toBeTruthy();
            expect(global.modal.text2Modal).toHaveBeenCalledTimes(2);
        });

        test('should handle premium user flow: unlock shortcuts and show feedback links', () => {
            chrome.runtime.sendMessage.mockImplementation((msg, cb) => cb({ result: { isPremium: true } }));
        ...
        test('should handle free user flow: keep shortcuts locked', () => {
            ...
            Array.from(shortcutTip).forEach(el => expect(el.classList.contains('premium-only')).toBe(true));
            expect(global.modal.text2Link).toHaveBeenCalledWith('premiumNote', 'ExtensionPay', 'https://extensionpay.com/');
        });
    ...
    describe('Constructor and Error Handling', () => {
        test('should create instance with all required methods', () => {
            ...
            ['checkPay', 'updateShortcutDisplay', 'updateNoteDisplay', 'calcTrialEndDate'].forEach(method => {
