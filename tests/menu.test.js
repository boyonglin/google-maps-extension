...
describe("ContextMenuUtil", () => {
  ...
  describe("createContextMenu", () => {
    ...
    test("should add premium-option class when user cannot tidy", async () => {
      ...
      const tidyOption = Array.from(menu.querySelectorAll(".context-menu-item")).find((item) =>
        item.textContent.includes("Tidy Locations")
      );
  ...
  describe("openAllUrls", () => {
    ...
    test("should open in group when canGroup is true", async () => {
      ...
      chrome.runtime.sendMessage.mockImplementation((msg, callback) => {
        if (callback) callback({ canGroup: true });
      });
  ...
  describe("findMatchingElement", () => {
    ...
    test("should return undefined for no match", () => {
      const element = ContextMenuUtil.findMatchingElement(
        "Completely Wrong",
        elementMap,
        elementsList
      );
